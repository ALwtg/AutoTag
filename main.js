...
async function exportBatchImageLabels(options = {}) {
    ...
    // Default behavior: export txt labels and classes.txt
    // If options.onlyClasses is true, only export classes.txt (handled by caller combining zips usually, but here we can support)
    
    const classes = getAllClasses();
    
    // Always include classes.txt if requested or default
    if (options.includeClasses !== false && classes.length > 0) {
        zip.file('classes.txt', classes.join('\n'));
    }

    if (options.onlyClasses) {
        // Just return zip with classes
    } else {
        // ...
...
async function exportBatchTaggedImages() {
    if (!state.imageResults || state.imageResults.length === 0) return alert('暂无图片标注结果');
    const zip = new JSZip();
    const folder = zip.folder('tagged_images');
    
    // We need to redraw each image with boxes
    const canvas = document.createElement('canvas');
    const ctx = canvas.getContext('2d');
    
    for (const item of state.imageResults) {
        if (!item.dataUrl) continue; // Should have dataUrl from processing
        ...
        item.annotations.forEach(ann => {
            drawStyledAnnotation(ctx, ann, canvas.width, canvas.height, rect);
        });
    ...
    const a = document.createElement('a');
    a.href = URL.createObjectURL(blob);
    a.download = `tagged_images_${Date.now()}.zip`;
    a.click();
}

async function exportBatchVideoFrames(options = {}) {
    if (!state.videoResults || state.videoResults.length === 0) return alert('暂无视频标注结果');
    const zip = new JSZip();
    const imgFolder = options.includeImages !== false ? zip.folder('images') : null;
    const lblFolder = options.includeLabels !== false ? zip.folder('labels') : null;
